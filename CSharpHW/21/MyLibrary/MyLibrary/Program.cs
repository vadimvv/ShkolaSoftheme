...
namespace MyLibrary
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            DateTime xmlTimer, xmlTimer2,
                    binTimer, binTimer2,
                    jsonTimer, jsonTimer2;
            ...
            using (FileStream fsXml = new FileStream("books.xml", FileMode.OpenOrCreate))
            {
                xmlTimer = DateTime.Now;
                foreach (var b in homeLibrary.BookList())
                {
                    formatter.Serialize(fsXml, b);
                }
                xmlTimer2 = DateTime.Now;
            }

            using (FileStream fsJson = new FileStream("book.json", FileMode.OpenOrCreate))
            {
                jsonTimer = DateTime.Now;
                foreach (var b in homeLibrary.BookList())
                {
                    jsonFormatter.WriteObject(fsJson, b);
                }
                jsonTimer2 = DateTime.Now;
            }

            using (FileStream fsBin = new FileStream("book.dat", FileMode.OpenOrCreate))
            {
                binTimer = DateTime.Now;
                foreach (var b in homeLibrary.BookList())
                {
                    formatter.Serialize(fsBin, b);
                }
                binTimer2 = DateTime.Now;
            }
            ...
            Console.WriteLine("XML timer: " + (xmlTimer2 - xmlTimer).Ticks +
                "\nJSON timer: " + (jsonTimer2 - jsonTimer).Ticks +
                "\nBinnary timer: " + (binTimer2 - binTimer).Ticks);
            Console.ReadLine();
        }
